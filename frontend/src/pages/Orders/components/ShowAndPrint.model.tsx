...
import backendUrl from "../../../provider/queries/config"

const ShowAndPrintModel = ({ setVisible, visible,id,paymentStatus,items }:any) => {
  ...
  const payment_stripe= async()=>{
    const stripe = await loadStripe("pk_test_51Pi0gZ2MYvAYdxOMJEk3yxDI2L9gmuQfUrEMEEDcrEeQ8jfSwdYtYE3NWG7rBY5bsvPGGfM3XMOJa7UgRit5OX0600HPoJpync")
    const body = {
      products:items,
      orderId:id
    }
    ...
    const response  = await fetch(`${backendUrl}/pay/create-checkout-session`,{
      method:"POST",
      headers:headers,
  ...
  return (
    <>
              ...
              <tbody>
                {data.Items && data.Items.length > 0 && data.Items.map((c: OrderDoc, i:number) => {
                  return <tr key={i} className='py-2'>
                    ...
                    <td className='border text-center py-2 capitalize'>{c.item_name}</td>
        ...
        <footer className='gap-5'>
          ...
          >Generate</button>
          {paymentStatus!="Completed"&&<button className='px-6 py-2 ml-3 outline-none bg-red-500 rounded-md text-white' 
            onClick={payment_stripe}
